...
value class R<VALUE:Any> @PublishedApi internal constructor(@PublishedApi internal val value:Any){
    companion object{
        /** 정상인 값을 생성함 */
     ...
     */
    inline fun map(crossinline block:(VALUE)->VALUE):R<VALUE> = when(value){
        is Throwable -> this
        is Function0<*> -> R({
            try{block((value as ()->VALUE)())}catch(e:Throwable){e}
        })
        else -> R(try{block(value as VALUE)}catch(e:Throwable){e})
    }
    /** 실패값을 반드시 복원할 수 있는 정책이 있는 경우 복원용 람다를 통해 현재 상태를 나타내는 예외로부터 값을 만들어냄 */
    inline operator fun invoke(block:(Throwable)->VALUE):VALUE = when(value) {
        is Throwable -> block(value)
        is Function0<*> -> try{(value as ()->VALUE)()}catch(e:Throwable){block(e)}
        else -> value as VALUE
    }
    /** 정상인 값은 반환되지만 비정상인 값은 null이 됨. 지연연산 설정 시 이 시점에 해소됨*/
    inline operator fun invoke():VALUE? = when(value){
        is Throwable -> null
        is Function0<*> -> try{(value as ()->VALUE)()}catch(_:Throwable){null}
        else -> value as VALUE
    }
}