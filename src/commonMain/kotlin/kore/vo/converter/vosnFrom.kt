...
import kore.error.E
import kore.vo.VO
import kore.vo.converter.VOSN.Cursor
import kore.vo.converter.VOSN.Cursor.DecodeNoListTeminator
import kore.vo.converter.VOSN.STRINGLIST_EMPTY_C
import kore.vo.converter.VOSN.decodeString
...
internal object VOSNfrom{
    ...
    internal fun <V:VO> vo(cursor:Cursor, vo:V):Wrap<V>{
        ...
        (VO.fields(vo.type) ?: return W(VOSNto.ToVONoInitialized(vo))).forEach{key->
            val field = fields[key] ?: return W(VOSNto.ToVONoInitialized(vo))
            when{
                cursor.isEnd->{}
