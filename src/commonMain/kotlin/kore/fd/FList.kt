...
/**-----------------------------------------------------------------*/
tailrec fun <ITEM:Any, ACC:Any> FList<ITEM>.fold(base:ACC, block:(ACC, ITEM)->ACC):ACC = when(this){
    is Cons -> tail.fold(block(base, head), block)
    is Nil -> base
}
inline fun <ITEM:Any> FList<ITEM>.reverse(): FList<ITEM> = fold(FList.invoke()){ acc, it->Cons(it, acc)}
...
@PublishedApi internal tailrec fun <ITEM:Any> FList<ITEM>._dropWhileIndexed(index:Int, block:(Int, ITEM)->Boolean): FList<ITEM> = if(this is Cons && block(index, head)) tail._dropWhileIndexed(index + 1, block) else this
fun <ITEM:Any> FList<ITEM>.dropWhileIndexed(block:(Int, ITEM)->Boolean):FList<ITEM> = _dropWhileIndexed(0, block)

@PublishedApi internal tailrec fun <ITEM:Any> FList<ITEM>._dropLast(n:Int):FList<ITEM> = when(this){
    is Nil -> reverse()
    is Cons -> if(n == 0) reverse() else tail._dropLast(n - 1)
}
fun <ITEM:Any> FList<ITEM>.dropLast(n:Int = 0):FList<ITEM> = reverse()._dropLast(n)
        //= if(n == 0) this else dropLast().dropLast(n - 1)
//tailrec fun <ITEM:Any> FList<ITEM>.dropLastWhile(block:(ITEM)->Boolean): FList<ITEM>
//        = if(this is Cons && block(_head)) dropLast().dropLastWhile(block) else this
//@PublishedApi internal tailrec fun <ITEM:Any> FList<ITEM>._dropLastWhileIndexed(index:Int, block:(Int, ITEM)->Boolean): FList<ITEM>
//        = if(this is Cons && block(index, _head)) dropLast()._dropLastWhileIndexed(index + 1, block) else this
//fun <ITEM:Any> FList<ITEM>.dropLastWhileIndexed(block:(Int, ITEM)->Boolean): FList<ITEM> = _dropLastWhileIndexed(0, block)
...
//
//
//fun <ITEM:Any> FList<FList<ITEM>>.flatten(): FList<ITEM> = when(this){
//    is Nil -> this
//    is Cons -> drop(1).fold(_head){ acc, it->
//        acc.append(it)
//    }
//}
