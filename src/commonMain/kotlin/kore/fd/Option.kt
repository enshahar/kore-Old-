...
//fun <VALUE:Any> FList<Option<VALUE>>._sequenceOption(acc: Option<FList<VALUE>>): Option<FList<VALUE>>
//    = when(this){
//    is FList.Nil -> acc
...
//    }
//}
//inline fun <VALUE:Any> FList<Option<VALUE>>.sequenceOption(): Option<FList<VALUE>> = _sequenceOption(Option(FList()))
//
//   //= reverse().fold(Option(List())){acc, it->it.map2(acc, ::Cons)}
//inline fun <VALUE:Any> FList<Option<VALUE>>.sequenceOptionT(): Option<FList<VALUE>>
//    = traverseOption{it}
//fun <VALUE:Any, OTHER:Any> FList<VALUE>.traverseOption(block:(VALUE)-> Option<OTHER>): Option<FList<OTHER>>
//    = when(this){
//        is FList.Nil -> Option(FList())
...
//}
//fun FList<Double>.average(): Option<Double> = when(val s = size){
//    0 -> Option()
//    else -> Option(sum() / s)
//}
//fun FList<Double>.variance(): Option<Double> = when(val avg = average()){
//    is Option.None ->avg
//    is Option.Some ->map{(it - avg.value).pow(2)}.average()
//}
//fun FList<Double>.varianceK(): Option<Double> = average().flatMap {
