...
////** append-----------------------------------------------------------------*/
fun <ITEM:Any> WList<ITEM>.appendW(list:Wrap<WList<ITEM>> = W{WList()}): Wrap<WList<ITEM>> = foldRightW(list){it, acc->W{Cons(it, acc)}}
fun <ITEM:Any> WList<ITEM>.append(list:WList<ITEM> = WList()): WList<ITEM> = foldRight(list){it, acc->Cons(it, W{acc})}
...
tailrec fun <ITEM:Any> WList<ITEM>.startsWith(target:WList<ITEM>):Boolean {
    return when(this) {
        is Cons ->{
            when(target){
